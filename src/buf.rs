use alloc::{self,allocator, heap};
use self::allocator::{Alloc, Layout};
use self::heap::Heap;
...
impl RawBuf {
    ...
    fn new(len: usize) -> RawBuf {
        unsafe {
            ...
            let bytes = Heap.alloc(Layout::from_size_align(len, refcount_len).unwrap()).unwrap();
            //if bytes == ptr::null_mut() { alloc::oom::oom() }
            *(bytes as *mut u64) = 1;
            RawBuf {
